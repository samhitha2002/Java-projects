...
public class WeatherGenerator {
    ...
    public static int[] oneMonthGenerator(double longitude, double latitude, int month, double[][] drywet, double[][] wetwet) {
        ...
        int[] Month = new int [numberOfDaysInMonth[month-2]];
        double probabilityOfRain = StdRandom.uniform(); 
        
        if ( 0 <= probabilityOfRain && probabilityOfRain < 0.5){
            Month[0] = WET;
        }else{
            Month[0] = DRY;
        }
        ...
        for (int i = 1; i <  Month.length; i++){
            double futureProbability = StdRandom.uniform();
            if(Month[i-1] == WET){
                for(int r = 0; r < wetwet.length; r++){
                    if((wetwet[r][0] == longitude) && (wetwet[r][1] == latitude)){
                        if(wetwet[r][month] >= futureProbability){
                            Month[i] = WET;
                        }else{
                            Month[i] = DRY;
                        }
                ...
                for (int r = 0; r < drywet.length; r++){
                    if((drywet[r][0] == longitude) && (drywet[r][1] == latitude)){
                        if(drywet[r][month] >= futureProbability){
                            Month[i] = WET;
                        }else{
                            Month[i] = DRY;
                        }
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        for (int i = 0; i < forecast.length; i++){
            if (forecast[i] == mode){
                countWetOrDry++;
                if (countWetOrDry > Consecutive){
                    Consecutive = countWetOrDry;
                }
            }else{
                countWetOrDry = 0;
            }
