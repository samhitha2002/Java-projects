...
public class RUMDbSearchEngine {
	...
	public void insertMoviesIntoHashTable ( String inputFile ) {
		...
		ArrayList<ArrayList<String>> file = readInputFile(inputFile);

		for(int i = 0; i < file.size(); i++){
			for(int j = 1; j < file.get(i).size(); j++){
				String value = isWord(file.get(i).get(j));
				Location place = new Location(file.get(i).get(0),j);
				
				if(value != null){
					if(!value.equals(isWord(value))){
						insertWordLocation(value, place);
					}else{
						value = isWord(value);
						insertWordLocation(value, place);
					}
	...
	public void insertWordLocation (String word, Location loc) {
		...
		if(getWordOccurrence(word) != null && getWordOccurrence(word).getWord().equals(word)){
			...
			int indexValue = hashFunction(word);

			if(hashTable[indexValue] == null){
				hashTable[indexValue] = insert;
			}else{
				insert.next = hashTable[indexValue];
				hashTable[indexValue] = insert;
			}
		}
		...
		if (getLoadFactor() > threshold) {
			...
			for (WordOccurrence first : hashTable) {
				...
				while(temp != null){
					...
					ptr.next = newHashTable[index];
					newHashTable[index] = ptr;
					ptr = temp;
					temp = temp.next;
				}
				if(first != null){
	...
	public WordOccurrence getWordOccurrence (String word) {
        ...
        // COMPLETE THIS METHOD
		int indexValue = hashFunction(word);
		for (WordOccurrence ptr = hashTable[indexValue]; ptr != null; ptr = ptr.next) {
	...
	public ArrayList<MovieSearchResult> createMovieSearchResult (String wordA, String wordB) {
        ...
        ArrayList<Location> aWordLoc = aWord.getLocations();
        ArrayList<Location> bWordLoc = bWord.getLocations();

        for(int i = 0; i < aWordLoc.size(); i++){
			boolean tracker = false;
			for(int j = 0; j < result.size(); j++ ){
				if(result.get(j).getTitle().equals(aWordLoc.get(i).getTitle())){
					result.get(j).addOccurrenceA(aWordLoc.get(i).getPosition());
					tracker = true;
					break;
				}
			}
			if(tracker == false){
				MovieSearchResult n = new MovieSearchResult(aWordLoc.get(i).getTitle());
				n.addOccurrenceA(aWordLoc.get(i).getPosition());
				result.add(n);
			}
        }
		for(int i = 0; i < bWordLoc.size(); i++){
			boolean tracker = false;
			for(int j = 0; j < result.size(); j++ ){
				if(result.get(j).getTitle().equals(bWordLoc.get(i).getTitle())){
					result.get(j).addOccurrenceB(bWordLoc.get(i).getPosition());
					tracker = true;
					break;
				}
			}
			if(tracker == false){
				MovieSearchResult n = new MovieSearchResult(bWordLoc.get(i).getTitle());
				n.addOccurrenceB(bWordLoc.get(i).getPosition());
				result.add(n);
			}
        }
		return result;
	...
	public void calculateMinDistance(MovieSearchResult msr){
		...
		if(msr.getArrayListA().isEmpty() || msr.getArrayListB().isEmpty()){
			...
			int minVal = Math.abs(msr.getArrayListB().get(0) - msr.getArrayListA().get(0));
			while(val1 < msr.getArrayListA().size() && val2 < msr.getArrayListB().size()){
				if(msr.getArrayListA().get(val1) < msr.getArrayListB().get(val2)){
					int newMin = Math.abs(msr.getArrayListB().get(val2) - msr.getArrayListA().get(val1));
					if(minVal > newMin){
						minVal = newMin;
					}
					val1++;
				}else{
					int newMin = Math.abs(msr.getArrayListA().get(val1) - msr.getArrayListB().get(val2));
					if(minVal > newMin){
						minVal = newMin;
					}
					val2++;
	...
	public ArrayList<MovieSearchResult> topTenSearch(String wordA, String wordB){
		...
		ArrayList<MovieSearchResult> sortResult = new ArrayList<MovieSearchResult>();
		ArrayList<MovieSearchResult> searchMovie = createMovieSearchResult(wordA, wordB);
		...
		for(int i = 0; i < searchMovie.size(); i++){
			calculateMinDistance(searchMovie.get(i));
		}
		
		Collections.sort(searchMovie);
		int i = 0;

		while(i < searchMovie.size()){
			if( i > 10){
				break;
			}
			if(searchMovie.get(i).getMinDistance() != -1){
				sortResult.add(searchMovie.get(i));
			}
			i++;
		}
		return sortResult;
